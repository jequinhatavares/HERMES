...
if "esp32" in board:
    board_id = "USB VID:PID=1A86:"
elif "nodemcu" in board:
    board_id = "USB VID:PID=10C4"
else:
    # default to esp32
    board_id = "USB VID:PID=1A86:"
    ...
    if simultaneous_upload_ports != None:  # check if defined
        ...
        if str(ports) == "AUTO":
            ...
            if "port': 'COM" in str(ports):
                ...
                port_list = []
                for port in ports:
                    if board_id in str(port["hwid"]):
                        port_list.append(port["port"])
                print(f"-> filtered ports for board {board}: " + str(port_list))
            else:
                # macOS
