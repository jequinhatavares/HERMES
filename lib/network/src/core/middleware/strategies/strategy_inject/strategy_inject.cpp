...
void influenceRoutingStrategyInject(char* messageEncodeBuffer,size_t encodeBufferSize,char* dataMessagePayload,uint8_t* finalDestination){
    ...
    sscanf(dataMessagePayload, "%*d %*u.%*u.%*u.%*u %hhu.%hhu.%hhu.%hhu",&originalDestination[0],&originalDestination[1],&originalDestination[2],&originalDestination[3]);
    ...
    if(isIPEqual(originalDestination,bestMetricIP) || !findBestMetric){
        ...
        if(nextHopIP != nullptr){
            ...
            sendMessage(nextHopIP, dataMessagePayload);
            LOG(MESSAGES,INFO,"Sending [DATA] message: \"%s\" to: %hhu.hhu.%hhu.%hhu\n",dataMessagePayload,originalDestination[0],originalDestination[1],originalDestination[2],originalDestination[3]);
    ...
    encodeTunneledMessage(messageEncodeBuffer,encodeBufferSize, myIP,bestMetricIP,dataMessagePayload);
