...
void handleMessageStrategyInject(char* messageBuffer, size_t bufferSize){
    ...
    if(injectType == INJECT_NODE_INFO){
        ...
        while (token != NULL) {
            ...
            //Decode and save the metric in the table
            registerInjectMetric(nodeIP,token+nChars);
...
void influenceRoutingStrategyInject(char* messageEncodeBuffer,size_t encodeBufferSize,char* dataMessagePayload,uint8_t* finalDestination){
    ...
    // If the best-metric IP matches the original destination, or if no best-metric node is found,
    // forward the message directly to its final destination without tunneling.
    if(isIPEqual(finalDestination,bestMetricIP) || !findBestMetric){
        //Send the message to the original destination
        nextHopIP = (uint8_t *) findRouteToNode(finalDestination);
        if(nextHopIP != nullptr){
            LOG(MESSAGES,INFO,"Encoded message: %s\n",dataMessagePayload);
            encodeDataMessage(messageEncodeBuffer,encodeBufferSize,dataMessagePayload,myIP,finalDestination);
            sendMessage(nextHopIP, messageEncodeBuffer);
            LOG(MESSAGES,INFO,"Sending [DATA] message: \"%s\" to: %hhu.%hhu.%hhu.%hhu\n",messageEncodeBuffer,finalDestination[0],finalDestination[1],finalDestination[2],finalDestination[3]);

        }else{
            LOG(MIDDLEWARE,ERROR,"ERROR: Trying to send message to: %hhu.%hhu.%hhu.%hhu but did not find path to node\n",finalDestination[0],finalDestination[1],finalDestination[2],finalDestination[3]);
        }
        return;
    }
    ...
    encodeDataMessage(largeSendBuffer, sizeof(largeSendBuffer),dataMessagePayload,myIP,finalDestination);
