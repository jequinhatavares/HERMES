...
void shutdownHandler(int sig) {
    if (sig == SIGINT) {
        printf("\n[CATCH SIGNAL] SIGINT received, shutting down...\n");
    } else if (sig == SIGTERM) {
        printf("\n[CATCH SIGNAL] SIGTERM received, shutting down...\n");
    }
    running = false;
    network.stop();
}
...
void setup(){
    ...
    // Register Shutdow handlers
    signal(SIGINT, shutdownHandler);// Catch SIGINT (generated by Ctrl+C in the terminal)
    signal(SIGTERM, shutdownHandler); // Catch SIGTERM (generated by `kill <pid>` or system shutdown commands)
